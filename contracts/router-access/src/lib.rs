...
mod tests {
    ...
    fn test_blacklisted_address_cannot_regain_super_admin_via_transfer() {
        ...
        // A blacklisted caller can never pass the super-admin gate for any
        // privileged action, using set_role_admin as a probe. require_super_admin
        // checks blacklist status before identity (see its doc comment), so a
        // caller who is both blacklisted and not the super-admin is rejected
        // as Blacklisted rather than Unauthorized — either way the call is
        // correctly denied.
        let role = String::from_str(&env, "operator");
        let victim = Address::generate(&env);
        let probe = client.try_set_role_admin(&admin_a, &role, &victim);
        assert_eq!(probe, Err(Ok(AccessError::Blacklisted)));
    }
    ...
    fn test_set_role_parent_rejects_chain_deeper_than_max_hierarchy_depth() {
        ...
        // Build role-0 -> role-1 -> ... -> role-15  (15 edges, all accepted).
        for i in 0usize..15 {
            client.set_role_parent(&admin, &roles[i], &roles[i + 1]);
        }

        // A role outside the existing chain, made a child of role-0, must walk
        // the full 16-node chain (role-0..role-15) before it could find a free
        // slot — hitting depth == MAX_HIERARCHY_DEPTH inside
        // ensure_no_role_parent_cycle without ever revisiting `outsider`
        // itself, so this is genuinely "too deep", not a cycle.
        let outsider = String::from_str(&env, "role-outsider");
        let result = client.try_set_role_parent(&admin, &outsider, &roles[0]);
